  ...
  for (Patch *aPatch in patches) {
    ...
    if (start_loc == NSNotFound) {
      ...
      if (end_loc == NSNotFound) {
        //text2 = [textMutable substringWithRange:NSMakeRange(start_loc,MIN(text1.length, textMutable.length)- start_loc)];
      } else {
                             ...
                             error:(NSError **)error;
{
  ...
  while (textPointer < text.count) {
    ...
    NSString* a = [textAtTextPointer substringFromIndex:1];
      line = [a
